/* 🎨 SISTEMA DE ICONOS CENTRALIZADO - VERSIÓN CORREGIDA */
/* Todos los iconos SVG de la aplicación en un solo lugar */

/**
 * Componente genérico para crear iconos SVG
 */
export const createIconSVG = (path, viewBox = '0 0 24 24', className = '') => {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}" class="${className}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${path}</svg>`;
};

/* 🎨 Librería completa de iconos SVG */
export const ICONS = {
  // Navegación
  ...
  // Productos
  Shirt: '<path d="M20.38 3.46 16 2a4 4 0 0 0-8 0L3.62 3.46a2 2 0 0 0-1.34 2.23l.58 3.47a1 1 0 0 0 .99.84H6v10c0 1.1.9 2 2 2h8a2 2 0 0 0 2-2V10h2.15a1 1 0 0 0 .99-.84l.58-3.47a2 2 0 0 0-1.34-2.23z"></path>',
  ...
  LineChart: '<path d="M3 3v18h18"></path><path d="M19 9l-4 4-4-4-4 4-4-4"></path>',
...
/* 🎨 Obtener SVG de icono como string */
export const getIconSVG = (iconName, className = '') => {
  const path = ICONS[iconName];
  if (!path) {
    console.warn(`⚠️ Icono no encontrado: ${iconName}`);
    return '';
  }
  return createIconSVG(path, '0 0 24 24', className);
};

/* 🎨 Crear elemento HTML de icono */
export const createIconElement = (iconName, className = '') => {
  const svg = getIconSVG(iconName, className);
...
/* 🎯 Componentes React para iconos */
export const Icon = ({ name, className = '', size = 24, color = 'currentColor' }) => {
  const path = ICONS[name];
  if (!path) {
    console.warn(`⚠️ Icono no encontrado: ${name}`);
    return null;
  }
