...
export const CartProvider = ({ children }) => {
  ...
  const customTotal = cart.reduce((acc, item) => {
    if (item.isCustom) {
      ...
      const size = item.details?.size || 'M';
      const quantity = item.quantity || 1;
      // Para items personalizados, usar precio base
      const basePrice = 60; // Precio base personalizado
  ...
  const catalogTotal = catalogCart.reduce((acc, item) => {
    ...
    const quantity = item.quantity || 1;
    // Para catálogo, usar precios base según tipo
    let basePrice = 55; // Precio base por defecto
    if (type === 'saco') basePrice = 189;
    else if (type === 'blusa') basePrice = 77;
    else if (type === 'solera') basePrice = 93;
  ...
  return (
    React.createElement(CartContext.Provider, { value }, children)
  );
};
